// api-client/modules/trips.api.ts
import {
  CreateTripPayload,
  SearchTripQuery,
  Trip,
...
export const tripsApi = {
  ...
  search: async (params: TripSearchParams): Promise<Trip[]> => {
    try {
      const response = await http.get<TripResponse>('/trips/search', { params });
  ...
  /* ===== SEARCH BY PROVINCES ===== */
  searchByProvinces: async (fromProvince: string, toProvince: string, date: string): Promise<Trip[]> => {
    try {
  ...
  searchByRoute: async (fromId: string, toId?: string): Promise<Trip[]> => {
    try {
      ...
      const response = await http.get<TripResponse>('/trips/search/route', { params });
      return response?.data || [];
    } catch (error) {
  ...
  getTripByIdSafe: async (id: string): Promise<{ trip: Trip | null; error: string | null }> => {
    try {
  ...
  calculateTripDuration: (departureTime: string | Date | undefined, arrivalTime: string | Date | undefined): string => {
