...
export function RouteManagement() {
  ...
  const [selectedTripForDriver, setSelectedTripForDriver] = useState<
    string | null
  >(null);
  ...
  const calculateStatsFromTrips = (tripsData: Trip[]) => {
    ...
    const scheduledTrips = tripsData.filter(
      (trip: Trip) => trip.status === "scheduled",
    ).length;
    const runningTrips = tripsData.filter(
    ...
    tripsData.forEach((trip: Trip) => {
      const totalSeats =
        trip.totalSeats || (trip.vehicleId as any)?.totalSeats || 40;
  ...
  const getFilteredTrips = () => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((trip) => {
        ...
        const fromName =
          route?.fromLocationId?.name?.toLowerCase() ||
          route?.from?.name?.toLowerCase() ||
          "";
        const toName =
          route?.toLocationId?.name?.toLowerCase() ||
          route?.to?.name?.toLowerCase() ||
          "";
        const routeName = `${fromName} ${toName}`;

        const vehiclePlate =
          (trip.vehicleId as any)?.vehicleNumber?.toLowerCase() || "";
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p style={{ color: "var(--obtp-muted2)", marginTop: 6 }}>
            Quản lý lịch trình và chuyến đi của nhà xe
          </p>
        </div>
        <div className="flex items-center space-x-3">
          <button
          ...
          <button onClick={handleCreateNewTrip} className="obtp-btn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="obtp-card obtp-card-strong" style={{ padding: 14 }}>
          <div className="flex items-center space-x-3">
      ...
      {/* Search and Filter */}
      <div
        className="obtp-card obtp-card-strong"
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>{displayedTrips.map(renderTripRow)}</tbody>
            </table>
          </div>
        ) : null}
      </div>
