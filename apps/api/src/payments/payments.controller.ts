...
export class PaymentsController {
  ...
  // WEBHOOK (PUBLIC ENDPOINT CALLED BY PAYOS SERVER)
  @Post('webhook')
  @HttpCode(HttpStatus.OK)
  // Use Zod Pipe to ensure basic structure safety
  @UsePipes(new ZodValidationPipe(PayOSWebhookSchema))
  async handleWebhook(@Body() payload: PayOSWebhookPayload) {
    // Background process to avoid timeout?
    // Since it's critical payment logic, await is safer to ensure db updated.
