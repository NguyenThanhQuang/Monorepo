...
export function UserManagement() {
    ...
    return users.filter((user) => {
      ...
      if (filterRole !== "all") {
        if (filterRole === UserRole.USER) {
          matchesRole = user.roles.includes(UserRole.USER) && user.roles.length === 1;
        } else {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-96 bg-red-50/50 rounded-3xl border border-red-100">
      ...
      <button onClick={() => refetch()} className="px-6 py-2 bg-blue-600 text-white rounded-xl shadow-lg shadow-blue-200 font-bold uppercase">
        {t("retry")}
      </button>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-blue-100 flex items-center justify-center text-blue-600 shadow-sm shadow-blue-500/10"><UsersIcon /></div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-green-100 flex items-center justify-center text-green-600 shadow-sm shadow-green-500/10"><CheckCircle /></div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-red-100 flex items-center justify-center text-red-600 shadow-sm shadow-red-500/10"><Ban /></div>
            ...
            >
              ...
              <div className="flex items-center gap-6">
                <div className="text-right hidden lg:block">
                  <p className="text-[10px] text-gray-400 uppercase font-black tracking-tighter">{t("totalSpent")}</p>
                  <p className="font-black text-blue-600 dark:text-blue-400">{formatCurrency(user.totalSpent || 0)}</p>
                </div>
                
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border font-black text-[10px] uppercase tracking-tighter ${user.isBanned ? 'bg-red-50 border-red-100 text-red-600' : 'bg-green-50 border-green-100 text-green-600'}`}>
